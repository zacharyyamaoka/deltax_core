...
class Scurve_Interpolator:
    ...
    def find_sync_point(self, x1, y1, z1, x2, y2, z2, vel, angle, time_offset=0):
        """
        vel and angle are of p2. 
        """
        
        # internally use mm and rad
        x1, y1, z1, x2, y2, z2, vel = x1*1000, y1*1000, z1*1000, x2*1000, y2*1000, z2*1000, vel*1000
        start_time = time.time()
        # rad = math.radians(angle)
        rad = angle

        Cmx, Cmy = self.find_short_point(x1, y1, x2, y2, self.max_vel, vel, angle) # take an inital good guess
...
def main(args=None):
    ...
    # print(moving.find_sync_point(0, 0, -0.9, 0.1, 0, -0.9, vel=0.2, angle=0, time_offset=0 ))
    # print(moving.find_sync_point(0, 0, -0.9, 0.1, 0, -0.9, vel=0.1, angle=0, time_offset=0 ))
    # print(moving.find_sync_point(0, 0, -0.9, 0.1, 0, -0.9, vel=0.1, angle=0, time_offset=2 ))
